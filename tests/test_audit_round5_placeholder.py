...
class TestWarnOnMalformedInferenceLLM:
    ...
    def test_fstring_simples_tambem_detectado(self):
        ...
        try:
            # Estilo de f-string com substituição E provider —
            # `_warn_on_malformed_inference_llm` precisa ter `:` para
            # chegar no check de placeholder (caminho "no colon" sai antes).
            _warn_on_malformed_inference_llm(
                "{INFERENCE_LLM}:gpt-4o-mini"
            )
            assert any(
                "UN-EXPANDED" in m for m in captured
